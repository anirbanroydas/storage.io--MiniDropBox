...
sigdirectory = "/home/cdot/Documents/Storage.0/sigfilestore/"
...
class FileSplitter:
    ...
    def split(self):
        ...
        # Create the signature of the file when its created or added for the first time
        f.seek(0)
        sigfilename = filehash+'-s'
        sigfilepath = os.path.join(sigdirectory,sigfilename)
        sfile = file(sigfilepath,'wb')
        sig = librsync.signature(f,sfile)

        print "\n sending signature file 'sfile' to S-Server...."
        sfile.close()
...
class FileModify :
    ...
    def send_filehash_mserver(self,filehash):
        ...
        if cur.rowcount>0 :
            ...
            # Access the version no. so that it can be updated later
            q= "SELECT version from filesystem where uID = %d and deviceID = %d and path = '%s' and filename = '%s' " % (uId,deviceId,path,filename)
            cur.execute(q)
            if cur.rowcount>0:
            	ver = cur.fetchone()[0]
            else:
            	print "\n cant access particular file in that path \n"
            ...
            commitQ = "INSERT into filesystem (uID,deviceID,path,filename,filehash,version) values (%d,%d,'%s' ,'%s', '%s',%d)" % (uId,deviceId,path,filename,fh,ver+1)
            cur.execute(commitQ)
